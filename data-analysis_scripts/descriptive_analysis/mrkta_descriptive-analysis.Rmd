...
### Summarise sales (boxplot, table)
```{r salessummary,echo = F, warning = F}
sales %>% 
  select(species, sales) %>% 
  group_by(species) %>% 
  summarise( min = min(sales), 
             lq = quantile(sales,0.25),
             median = median(sales),
             mean = mean(sales), 
             uq = quantile(sales,0.75),
             max = max(sales)) %>% 
  paged_table()
...
ggplot(sales,
       ...
       aes(x=reorder(species,-sales,FUN=median, desc.=F), y=sales))+
  ...
  labs(x="Species", y = "Small ruminant sales")+
...
- Small ruminant sales per market, grouped by region:
