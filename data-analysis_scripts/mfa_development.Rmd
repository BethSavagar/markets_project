...
mrkts_master_noMtwara <- read_csv(here("data/data-cleaning_data/mrkts_origdest_noMtwara.csv")) %>%
  select(-`...1`)
...
mrkts_PCA <- mrkts_master_complete %>%
  select(
    # envirchar_s (s)
    humpop_density1km,
    town.distkm,
...
fviz_pca_var(res.pca,
             col.var = "contrib", # Color by contribution to the PCA
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE # Avoid text overlapping
             )
...
hcpc_result <- HCPC(res.mfa, axes = 1:5)
# Plot the dendrogram
fviz_dend(hcpc_result, cex = 0.5)
