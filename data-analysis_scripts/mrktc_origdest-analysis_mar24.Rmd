...
origin_moves <- ggplot(nodes) + 
  ...
  geom_point(aes(x = long, y = lat, col = nodetype)) # +
  # scale_size_continuous(guide = FALSE, range = c(1, 6)) # +    # scale for node size
  # geom_text(aes(x = long, y = lat, label = ward),             # draw text labels
...
edges <- mrktOD_df %>%
  ...
  count() %>%
  select(from = destid, to = mrktid, weight = n)
