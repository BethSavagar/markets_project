...
mrktc_ward_gps <- mrktc_wards %>%
  st_drop_geometry() %>%
  mutate(mrktlong = st_coordinates(mrktc_wards)[,1],
         mrktlat = st_coordinates(mrktc_wards)[,2]) %>%
...
dist_exchange_df <- fullnet %>%
  # filter flow == origmrkt
  # filter flow == mrktdest
  mutate(distexchange = if_else(od.district == mrkt.district, "within", "other")) %>%
...
ggarrange(dist_within, dist_outside, ncol = 1)
```
