...
## 3) Market Survey B, Market Observations:
- For purpose of market type definition and MVA, Market Survey B provides:
    - Number of animals at the market by age and species group:
        - sheep <1y, sheep >1y, goat <1y, goat >1y
...
mrktb_summary <- mrktb_info %>%
  select(fid,ends_with(".final"),market.type, starts_with("sheep."), starts_with("goat."), - "date.final") %>%
  rowwise() %>% 
  mutate(num.animals.all = sum(c_across(`sheep.0to12`:`goat.adult`), na.rm = T)) %>%
  ungroup() %>%
  mutate(prop.sheep.0to12 = sheep.0to12/num.animals.all,
         prop.sheep.adult = sheep.adult/num.animals.all,
         prop.goat.0to12 = goat.0to12/num.animals.all,
         prop.goat.adult = goat.adult/num.animals.all) %>%
  select(ends_with(".final"), "num.animals.all", starts_with("prop."))

mrktb_forjoin <- mrktb_summary %>%
  rowwise() %>%
  mutate(prop.animal.main = names(mrktb_summary %>% select(prop.sheep.0to12:prop.goat.adult))[which.max(c_across(c(prop.sheep.0to12:prop.goat.adult)))],
         prop.animal.main = gsub("prop.", "", prop.animal.main)) %>%
  ungroup()

mrkts_master_v3a <- mrkts_master_v2 %>%
  left_join(mrktb_forjoin,
...
mrkts_master_v3 <- mrkts_master_v3a %>%
