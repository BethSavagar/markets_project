...
# Same as CLEAN1 data but contains all fields from generalinfo survey
...
map_plots <- list()
regions <- mrkta_gps %>% distinct(region) %>% pull()
...
for(i in 1:length(regions)){
  ...
  map_plots[[i]] <- ggplot() +
    ...
    geom_sf(data = mrkta_sf %>% filter(region %in% region_i), 
            aes(col = ward), alpha = 0.5, size = 3)+
    geom_sf(data = mrktc_sf %>% filter(region %in% region_i), 
            aes(col = ward),  shape = 17, size = 1.5)+
