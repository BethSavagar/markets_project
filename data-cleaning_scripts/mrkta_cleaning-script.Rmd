...
# Filter dataframe to understand whether these are duplicates or an entry error
duplicate.markets <- mrkta_clean %>% group_by(market.name) %>% count() %>% filter(n>1) %>% pull(market.name)
duplicate.entries <- mrkta_clean %>% filter(market.name %in% duplicate.markets)

# Dutwa appears to be a common name for two different markets, here replace the market name with the village name (distinct for the two entries)
