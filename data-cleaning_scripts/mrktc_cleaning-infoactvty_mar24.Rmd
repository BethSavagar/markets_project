...
mrktc_all <- mrktc_info %>%
  select(-c(date, country, region, district, district20, ward, village, market, gps.coordinates.of.the.market)) %>%
  left_join(mrktc_gps %>% select(-unique.row.identifier.uuid.), by = "fid") %>%
